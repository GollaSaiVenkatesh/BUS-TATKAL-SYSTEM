            ...
            if($result1 & $result2){
                ...
                if (mysqli_query($db, $bus1)) {
                    ...
                    if(mysqli_query($db, $alter1) & mysqli_query($db, $alter2)){
                        ...
                        $currdate=date("Y-m-d");
                        if($date==$currdate)
                        $final1="SELECT bus_no,src_city,src_time,dst_city,dst_time,sum(stop_amnt) 'stop_amnt' from fare1 where fare1.src_time>'".$time."' group by bus_no;";
                        else
                        $final1="SELECT bus_no,src_city,src_time,dst_city,dst_time,sum(stop_amnt) 'stop_amnt' from fare1 group by bus_no;";
                      

                        $fareresult1=mysqli_query($db,$final1);
...
<head>
	<title>Table</title>
	<link rel="stylesheet" type="text/css" href="tablestyle1.css">
</head>
<body>
	<section>
	  ...
	  <div class="tbl-content">
	    <table cellpadding="0" cellspacing="0" border="0">
            <tbody>
                <?php
                    while($row = mysqli_fetch_assoc($fareresult1)){?>
                        <tr>
                            ...
                            <td>
	          	                <div class="button_cont" align="center">
                                    <?php
                                      echo'<a class="example_d" href="seatlayout1.php?bus_no='.$row['bus_no'].'&source='.$row['src_city'].'&destination='.$row['dst_city'].'&date='.$date.'">Book Now</a>'
                                    ?>
	          	                </div>
	                        </td>
                        </tr>
